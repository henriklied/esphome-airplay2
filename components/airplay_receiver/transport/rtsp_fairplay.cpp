#include "rtsp_fairplay.h"

#include <cstring>

#include "../allocator.h"

namespace esphome {
namespace airplay_receiver {

// Pre-computed FairPlay handshake replies, lifted verbatim from upstream.
// Round 1 selects by mode byte; fp_header prefixes the round-2 echo.

static const uint8_t fp_reply1[] = {
    };

static const uint8_t fp_reply2[] = {
    };

static const uint8_t fp_reply3[] = {
    };

static const uint8_t fp_reply4[] = {
    };

static const uint8_t fp_header[] = {0x46, 0x50, 0x4c, 0x59, 0x03, 0x01,
                                    0x04, 0x00, 0x00, 0x00, 0x00, 0x14};

int rtsp_fairplay_handle(const uint8_t *body, size_t body_len, uint8_t **response, size_t *response_len) {
  if (response == nullptr || response_len == nullptr) {
    return -1;
  }
  *response = nullptr;
  *response_len = 0;

  if (body == nullptr || body_len < 16) {
    return -1;
  }

  const uint8_t version = body[4];
  const uint8_t type = body[5];
  const uint8_t seq = body[6];
  const uint8_t mode = body[14];

  if (version != 3 || type != 1) {
    return -1;
  }

  if (seq == 1) {
    const uint8_t *reply = fp_reply1;
    switch (mode) {
      case 1:
        reply = fp_reply2;
        break;
      case 2:
        reply = fp_reply3;
        break;
      case 3:
        reply = fp_reply4;
        break;
      default:
        break;
    }
    uint8_t *buf = static_cast<uint8_t *>(airplay_alloc(FP_REPLY_SIZE, false));
    if (buf == nullptr) {
      return -1;
    }
    std::memcpy(buf, reply, FP_REPLY_SIZE);
    *response = buf;
    *response_len = FP_REPLY_SIZE;
    return 0;
  }

  if (seq == 3) {
    const size_t len = FP_HEADER_SIZE + FP_SETUP2_SUFFIX_LEN;
    uint8_t *buf = static_cast<uint8_t *>(airplay_alloc(len, false));
    if (buf == nullptr) {
      return -1;
    }
    std::memcpy(buf, fp_header, FP_HEADER_SIZE);
    if (body_len >= FP_SETUP2_SUFFIX_LEN) {
      std::memcpy(buf + FP_HEADER_SIZE, body + body_len - FP_SETUP2_SUFFIX_LEN, FP_SETUP2_SUFFIX_LEN);
    } else {
      std::memset(buf + FP_HEADER_SIZE, 0, FP_SETUP2_SUFFIX_LEN);
    }
    *response = buf;
    *response_len = len;
    return 0;
  }

  return -1;
}

}  // namespace airplay_receiver
}  // namespace esphome
