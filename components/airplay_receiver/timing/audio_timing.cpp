// airplay_receiver stream timing state (C++ port of rbouteiller/airplay-esp32
// main/audio/audio_timing.c, PR #130 reduced engine-v2 revision).
//
// Holds the state the RTSP layer sets and the audio engine reads: the sender's
// anchor, the latency figures advertised in SETUP, and the deferred-flush
// handshake.  No playout logic remains (that lives in audio_scheduler.cpp).
// Heap is routed through airplay_* where allocated; diagnostics via ESP_LOGx.
...
static const char *const TAG = "audio_time";
...
void audio_timing_set_anchor(audio_timing_t *timing,
  ...
  int64_t lead_ms = ((int64_t) network_time_ns -
                    ...
                    1000000LL;
  ESP_LOGI(TAG, "Anchor set: rtp=%" PRIu32 " lead=%lld ms ptp_locked=%d", rtp_time,
           (long long) lead_ms, timing->ptp_locked);
}
...
void audio_timing_set_playing(audio_timing_t *timing, bool playing) {
  ...
  ESP_LOGI(TAG, "set_playing: %s -> %s",
           timing->playing ? "playing" : "paused", playing ? "playing" : "paused");
