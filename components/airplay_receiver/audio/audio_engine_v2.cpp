// airplay_receiver audio engine v2 — orchestrator (C++ port).
//
// Simple, faithful translation of rbouteiller/airplay-esp32
// main/audio/audio_engine_v2.c into the ESPHome component. Upstream behaviour
// is preserved; the only substantive changes are:
//   * namespace esphome::airplay_receiver (types from the ported sibling
//     headers are referenced unqualified via the shared namespace);
//   * logs use esphome/core/log.h (ESP_LOGx) with file-scope TAG "audio_v2";
//   * heap is routed through ../allocator.h (airplay_*) as the shared contract;
//     this module performs no direct allocation — the sole heap it touches is
//     the audio_timeline slot pool, allocated inside audio_timeline_init() and
//     released by audio_timeline_deinit(), both of which route through airplay_*.
//
// The engine is the v2 ORCHESTRATOR: it owns the audio_epoch_t counter, the
// audio_timeline_t slot pool, the audio_clock_map_t and the audio_scheduler_t,
// and it wires the decode/playout clock domains. Decoded PCM reaches it through
// audio_engine_v2_push_pcm() / _push_pcm_wait() (called by the decode worker);
// the audio output backend pulls audio via audio_engine_v2_render().
...
static const char *const TAG = "audio_v2";
