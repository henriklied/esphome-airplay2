#pragma once
// airplay_receiver audio_timeline — decoded-frame ring + playout-reservation
// engine (engine v2).
//
// C++ port of rbouteiller/airplay-esp32 main/audio/audio_timeline.h from PR
// #130 (the engine-v2 re-cut that replaces the audio_buffer + audio_timing core
// for decoded-frame storage). The upstream public API is preserved 1:1
// (function names, parameter lists and struct layouts are unchanged) and the
// whole module is wrapped in esphome::airplay_receiver so it links cleanly
// beside the other ported slices.
//
// MEMORY POLICY (ALL heap routes through ../allocator.h):
//   * the PCM payload pool (`pcm_pool`) lives in PSRAM
//     -> airplay_alloc(pcm_bytes, realtime=false). It is a large, low-bandwidth
//     decoded-frame pool (capacity * frame_samples * 2 channels * sizeof(int16_t)),
//     so it must NOT occupy internal DRAM (mirrors upstream's CONFIG_SPIRAM
//     heap_caps_malloc(MALLOC_CAP_SPIRAM) / malloc() intent).
//   * the descriptor table (`desc`) stays in internal DRAM
//     -> airplay_calloc(max_capacity, sizeof(audio_timeline_desc_t), realtime=true).
//     It is small, is walked inside a spinlock (portENTER_CRITICAL), and must be
//     cache-resident so a 192-entry scan completes in microseconds -- exactly
//     upstream's calloc() (internal) placement.
//   * no malloc/calloc/new/free anywhere; every buffer is released with
//     airplay_free() (which is NULL-safe).
//
// Owning agent note (shared types): this module defines its own
// audio_timeline_desc_t / audio_timeline_diag_t / audio_timeline_t /
// audio_timeline_reservation_t. It does NOT use the shared audio_format_t /
// audio_stream_type_t / audio_stats_t / audio_stream_t / AudioEncrypt /
// audio_receiver_state_t, so it deliberately does NOT include audio_receiver.h,
// audio_receiver_internal.h, audio_stream.h, or crypto/crypto_module.h (those
// belong to other port slices and are never redefined here).
...
struct audio_timeline_reservation_t {
  uint16_t slot_index;
  bool valid;
