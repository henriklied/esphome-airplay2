...
// main/audio/audio_stream.c, engine-v2 / PR #130).
//
// Owns the audio_stream container and the shared RTP-timestamp gate plus the
// decode+timeline-push entry points that BOTH the realtime (UDP, type 96) and
// buffered (TCP, type 103) receivers run their accepted frames through. The
// realtime receiver (audio_stream_realtime.cpp) binds the socket and hands
// decrypted packets here after recovery/gating; the buffered receiver
// (audio_stream_buffered.cpp) demuxes TCP into audio_encoded_packet_t access
// units handed to the decode worker, whose task calls
// audio_stream_decode_encoded_packet() (this file) to do the actual
// decoder->timeline publish. All heap routes through airplay_* (../allocator.h).
//
// DECRYPT: done by the callers (audio_stream_realtime / audio_stream_buffered)
// through CryptoModule::audio_decrypt_rtp / audio_decrypt_buffered — audio_crypto.c
// was NOT ported (see audio_receiver.h). This base only runs the decode+queue
// stage after a frame is accepted.
...
audio_stream_t *audio_stream_create_realtime(void) {
  audio_stream_t *stream =
      static_cast<audio_stream_t *>(airplay_calloc(1, sizeof(*stream), true));
  if (!stream) {
    return nullptr;
  }
