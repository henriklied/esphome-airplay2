#pragma once
// airplay_receiver audio engine v2 — orchestrator header.
//
// C++ port of rbouteiller/airplay-esp32 main/audio/audio_engine_v2.h into the
// ESPHome component. The upstream public API is preserved 1:1 (function names,
// parameter lists and the audio_engine_v2_t struct layout are unchanged) and
// the whole module is wrapped in esphome::airplay_receiver so it links cleanly
...
//     this module performs no direct allocation of its own — the only heap ever
//     touched lives in the audio_timeline slot pool (audio_timeline_init), which
//     the timeline module routes through airplay_*.
//   * logs via esphome/core/log.h + a file-scope TAG
//   * upstream function signatures preserved
//
// ROLE: the v2 engine is the ORCHESTRATOR that wires the audio timeline
// (slot pool), scheduler (playout state machine), clock map (sender<->local
// time / RTP mapping) and epoch counter together, and drives the decode and
// playout clock domains. It does NOT own the decode worker; buffered/realtime
// decode tasks call push_pcm()/push_pcm_wait() on it.
//
// Sibling headers are pulled in with relative includes (not redefined here).
// `AUDIO_V2_MAX_CHANNELS` comes from audio_timeline.h (audio_clock_map.h /
// audio_scheduler.h all use the same set of constants).
