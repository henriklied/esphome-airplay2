// airplay_receiver audio receiver orchestrator (C++ port).
//
// Simple, faithful translation of rbouteiller/airplay-esp32
// main/audio/audio_receiver.c into the ESPHome component. Upstream behaviour is
// preserved; the only substantive changes are:
...
esp_err_t audio_receiver_init(void) {
  ...
  // Non-realtime (PSRAM-first, internal fallback for small sizes) ring/decrypt
...
void audio_receiver_set_anchor_time(uint64_t clock_id, uint64_t network_time_ns,
  ...
  // Window size for the upper RTP gate: 10 s of samples.
  ...
  // Path B: Anchor-change detection — the phone changed track with a PAUSE ->
  // RESUME cycle but no FLUSHBUFFERED.
