// airplay_receiver audio receiver orchestrator (C++ port of
// rbouteiller/airplay-esp32 main/audio/audio_receiver.c, PR #130).
//
// Faithful translation to the ESPHome component namespace. Upstream behaviour
// and the public audio_receiver_* API are preserved; the substantive changes
// are:
//   * namespace esphome::airplay_receiver
//   * heap routes through airplay_alloc/airplay_free (../allocator.h)
//   * logs use esphome/core/log.h (ESP_LOGx) with TAG "audio_recv"
//   * audio_encrypt_t -> the component's AudioEncrypt (crypto/crypto_module.h);
//     audio_crypto.c is NOT ported, so decryption goes through CryptoModule.
//   * the audio decoder/stream ownership is unchanged; PLAUSIBILITY of the
//     timing path is delegated to audio_engine_v2 (see engine_v2 helpers).
//
// Engine v2 (PR #130): playback scheduling is delegated to audio_engine_v2_*
// (audio_engine_v2.h, engine agent). The engine is created lazily on first use
// and kept for the session; audio_receiver_read() renders from it on every I2S
// refill. The buffered path additionally offloads decode to a decode worker
// task (audio_decode_worker.h) that feeds the engine timeline.
//
// CryptoModule hook: the receiver holds the injected crypto pointer in its
// state. The actual audio_crypto_decrypt_rtp/buffered call sites were in
// audio_stream_realtime.c / audio_stream_buffered.c (the stream agent's files);
// those must now call  state->crypto->audio_decrypt_rtp(...)  /
// state->crypto->audio_decrypt_buffered(...). This file does not itself
// decrypt; it owns the CryptoModule injection point and the AudioEncrypt state.
...
static const char *const TAG = "audio_recv";
...
// Single receiver instance (matches upstream's global receiver state).
static audio_receiver_state_t receiver{};

static void audio_receiver_reset_stats(void) {
  std::memset(&receiver.stats, 0, sizeof(receiver.stats));
}
...
// every I2S refill and freeing the timeline underneath it would race.  The slot
// pool is therefore sized once for the longest frame either codec uses, and
// switching streams only re-points the addressing quantum -- 192 slots hold
// ~4.5 s of AAC or ~1.5 s of ALAC.
static esp_err_t audio_receiver_ensure_engine_v2(audio_stream_type_t type) {
  ...
  if (receiver.engine_v2.timeline.frame_samples != frame_samples) {
    ...
    // re-cut moves every slot address, and a decode worker or the playback task
    // can still be copying through a pointer it took under the old stride.  A
    // stale epoch makes new pushes fail immediately, which lets the re-cut wait
    // the stragglers out instead of racing them.
...
// over from a previous session (or kept alive by another sender on the network)
// would otherwise map an AirPlay 1 anchor into the PTP timeline, and vice
// versa, putting every block decades from its real playout time.  Like
// compute_early_us() in audio_timing.c this must be re-read on every render:
// before a lock the offset is 0, and freezing that value places the anchor days
// from local time and wraps the int32 RTP delta.
...
esp_err_t audio_receiver_init(void) {
  ...
  if (!receiver.realtime_stream || !receiver.buffered_stream) {
    ...
    receiver.realtime_stream = nullptr;
    receiver.buffered_stream = nullptr;
    return ESP_ERR_NO_MEM;
  }
  ...
  std::strcpy(default_format.codec, "AppleLossless");
  ...
  // Non-realtime (PSRAM-first, internal fallback for small sizes) decrypt
  // scratch, mirroring the upstream CONFIG_SPIRAM + heap_caps_malloc_prefer
  // policy via the component allocator.
  receiver.decrypt_buffer = static_cast<uint8_t *>(
      airplay_alloc(receiver.decrypt_buffer_size, /*realtime=*/false));
...
void audio_receiver_set_encryption(const AudioEncrypt *encrypt) {
  ...
  if (encrypt) {
    ...
    receiver.realtime_stream->encrypt = {};
    receiver.buffered_stream->encrypt = {};
  }
}
...
void audio_receiver_set_anchor_time(uint64_t clock_id, uint64_t network_time_ns,
  ...
  // Window size for the upper RTP gate: 10 s of samples.  Large enough that a
  // normal 2-4 s pre-buffer passes, but small enough to reject stale frames
  ...
  //
  // The blanket gate (discard_all_until_anchor) blocks ALL frames from the TCP
  // task.  The per-RTP gates filter by timestamp range.  On single-core
  // ESP32-S2, ESP_LOGI can yield to the scheduler, so any gap between clearing
  // the blanket and arming the per-RTP gates lets the TCP task queue stale
  // frames.  Arm first, then open.
  ...
  // anchor.  See upstream comment for the full rationale.
  ...
  // Pin the PTP clock to the master announced by the anchor packet's clock_id.
  // Without this, ptp_clock can lock to any PTP master on the LAN (HomePods,
  // AppleTVs, NTP-PTP gateways) and produce offsets that have nothing to do
  // with the AirPlay sender's clock domain.  clock_id == 0 happens on the
  // AirPlay 1 NTP path; in that case leave the filter as-is.
  if (clock_id != 0) {
...
void audio_receiver_set_playing(bool playing) {
  ...
  if (!playing) {
    receiver.blocks_read_in_sequence = 0;
    // Snapshot the expected RTP position at the moment of pause so that Path B
    // in audio_receiver_set_anchor_time() can compare the next resume anchor
    // against the actual pause position.  See upstream comment for why.
...
void audio_receiver_set_client_control(uint32_t client_ip,
  ...
  std::memset(&receiver.client_control_addr, 0,
...
void audio_receiver_stop(void) {
  ...
  receiver.decode_worker = nullptr;
...
void audio_receiver_get_stats(audio_stats_t *stats) {
  ...
  std::memcpy(stats, &receiver.stats, sizeof(receiver.stats));
}
...
void audio_receiver_flush(void) {
  ...
  // tracking.  The sender will provide fresh anchor times after flush.  Also
  // disarm any pending deferred flush so it does not fire on the next track's
  // frames.
  audio_receiver_reset_engine_v2();
  audio_timing_reset(&receiver.timing);
  audio_receiver_reset_resend_state();
...
void audio_receiver_seek_flush(void) {
  ...
  // This covers the forward-seek case where the buffer is already empty by the
  // time SETRATEANCHORTIME arrives, so the seek-detection heuristic (which
  // needs oldest_rtp from the buffer) would otherwise miss arming it.
  receiver.arm_gate_on_next_anchor = true;
  // Reject ALL incoming frames until the next anchor.  Prevents stale TCP data
...
void audio_receiver_pause(void) {
  ...
  // throttles the sender.  On resume the phone sends a fresh SETRATEANCHORTIME
  // anchor that re-aligns the buffered frames to the correct wall-clock
  // position; no flush or offset compensation is needed.
