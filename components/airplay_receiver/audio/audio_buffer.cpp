...
//
// Port of rbouteiller/airplay-esp32 main/audio/audio_buffer.c into the ESPHome
// component. The upstream logic is preserved exactly; the differences are:
//   * everything is wrapped in esphome::airplay_receiver;
//   * logging uses esphome/core/log.h (ESP_LOGI/W/E) with a file-scope TAG;
//   * ALL heap routes through airplay_alloc/airplay_calloc/airplay_free
//     (../allocator.h) — never malloc/calloc/new/free.
//
// MEMORY POLICY (justified in the matching header): the PCM pool lives in
// PSRAM (realtime=false) because it is a large (~1.4 MiB) ring buffer that
// must stay off internal DRAM; the slot index arrays and the decode scratch
// buffer are small, spinlock-accessed and form the decode/DMA working set, so
// they stay in internal DRAM (realtime=true) exactly as upstream's malloc()
// placement intended.
...
static const char *const TAG = "airplay_buffer";
...
esp_err_t audio_buffer_init(audio_buffer_t *buffer) {
  ...
  /* Pool in PSRAM (realtime=false: large, low-bandwidth ring buffer). */
  buffer->pool =
      (uint8_t *) airplay_alloc((size_t) buffer->capacity * buffer->slot_size, false);
  if (!buffer->pool) {
    ESP_LOGE(TAG, "Failed to allocate pool in PSRAM (%zu bytes)",
             (size_t) buffer->capacity * buffer->slot_size);
    return ESP_ERR_NO_MEM;
  }

  /* Sorted index array + free stack (internal DRAM, small + spinlock-accessed). */
  buffer->sorted =
      (uint16_t *) airplay_alloc(buffer->capacity * sizeof(uint16_t), true);
  buffer->free_stack =
      (uint16_t *) airplay_alloc(buffer->capacity * sizeof(uint16_t), true);
  ...
  /* Temp assembly / decode buffer (internal DRAM: decode/DMA-scratch working
     set, matches upstream malloc() placement). */
  size_t max_pcm_bytes =
      (size_t) MAX_SAMPLES_PER_FRAME * AUDIO_MAX_CHANNELS * sizeof(int16_t);
  buffer->frame_buffer =
      (uint8_t *) airplay_alloc(sizeof(audio_frame_header_t) + max_pcm_bytes, true);
  ...
  ESP_LOGI(TAG, "Sorted buffer created: %d slots x %zu bytes = %zu bytes",
...
void audio_buffer_deinit(audio_buffer_t *buffer) {
  ...
  if (buffer->pool) {
    airplay_free(buffer->pool);
    buffer->pool = NULL;
  }
  airplay_free(buffer->sorted);
  buffer->sorted = NULL;
  airplay_free(buffer->free_stack);
  buffer->free_stack = NULL;

  if (buffer->frame_buffer) {
    airplay_free(buffer->frame_buffer);
