...
//
// Port of upstream main/audio/audio_resample.c. The fixed-ratio, interleaved
// int16 stereo resampler is the David Bryant sinc resampler (BSD license),
// vendored below in a file-local anonymous namespace so this translation unit
// stays self-contained (no separate build object). Its heap is routed through
// airplay_*: the malloc/calloc/free inside the vendored block are remapped to
// airplay_* wrappers and the few void* -> typed-pointer assignments carry C++
// casts. ENABLE_THREADS / ENABLE_EXTRAPOLATION are not defined, so all
// multithread / extrapolation code is compiled out.
//
// The public audio_resample_* API (declared in audio_resample.h, in
// esphome::airplay_receiver) is implemented BELOW the anonymous namespace so it
// has external linkage and matches the header declarations. The output engine
// re-initializes the resampler when the source or output rate changes.
...
static void *rs_malloc(size_t n) { return airplay_alloc(n, true); }
static void *rs_calloc(size_t n, size_t sz) { return airplay_calloc(n, sz, true); }
static void rs_free(void *p) { airplay_free(p); }
#define malloc rs_malloc
#define calloc rs_calloc
#define free rs_free
...
Resample *resampleInit(int numChannels, int numTaps, int numFilters,
  ...
  for (i = 0; i <= cxt->numFilters; ++i) {
    ...
    cxt->filters[i] = (artsample_t *) calloc(cxt->numTaps, sizeof(artsample_t));
  ...
  for (i = 0; i < numChannels; ++i) {
    cxt->buffers[i] = (artsample_t *) calloc(cxt->numSamples, sizeof(artsample_t));
  }
...
Resample *resampleFixedRatioInit(int numChannels, int numTaps, int maxFilters,
  ...
  if (sourceRate == floor(sourceRate) && destinRate == floor(destinRate) &&
    ...
    if (factor <= (unsigned long) maxFilters) {
...
#ifdef ENABLE_THREADS
  if (cxt->workers) {
    Resample *worker_contexts = (Resample *) calloc(cxt->numChannels, sizeof(Resample));
...
static const char *const TAG = "audio_resample";

/* Taps must be a multiple of 4 (resampler requirement). A 32-tap Blackman-
 * Harris windowed-sinc is a good quality/CPU trade-off for 44.1<->48 kHz. */
#define RESAMPLER_NUM_TAPS 32

static Resample *resampler_;
static uint32_t current_input_rate_;
static uint32_t current_output_rate_;
static int current_channels_;
static double fixed_ratio_;
static bool active_;

/* Persistent float conversion buffers (airplay-allocated once) */
static float *float_in_;
static float *float_out_;
static size_t float_in_cap_;   /* in samples (frames * channels) */
static size_t float_out_cap_;

static void ensure_float_bufs(size_t in_samples, size_t out_samples) {
  if (in_samples > float_in_cap_) {
    airplay_free(float_in_);
    float_in_cap_ = in_samples;
    float_in_ = static_cast<float *>(airplay_alloc(float_in_cap_ * sizeof(float), true));
  }
  if (out_samples > float_out_cap_) {
    airplay_free(float_out_);
    float_out_cap_ = out_samples;
    float_out_ = static_cast<float *>(airplay_alloc(float_out_cap_ * sizeof(float), true));
  }
}
...
bool audio_resample_init(uint32_t input_rate, uint32_t output_rate, int channels) {
  ...
  current_output_rate_ = output_rate;
  current_channels_ = channels;
  fixed_ratio_ = (double) output_rate / (double) input_rate;

  /* Compute the exact filter count for interpolation-free resampling: the
   * reduced rational ratio output_rate/input_rate. */
  ...
  resampler_ = resampleFixedRatioInit(channels, RESAMPLER_NUM_TAPS, max_filters, (double) input_rate,
                                      (double) output_rate, 0, flags);
  ...
  /* Pre-allocate float buffers for a typical frame size (352 + margin). */
  size_t typical_in = 400 * (size_t) channels;
  size_t typical_out = (size_t) (400.0 * fixed_ratio_ + 16) * (size_t) channels;
  ensure_float_bufs(typical_in, typical_out);

  if (float_in_ == nullptr || float_out_ == nullptr) {
  ...
  active_ = true;
  ESP_LOGI(TAG, "Resampler: %lu -> %lu Hz, taps=%d, filters=%d, interp=%s", (unsigned long) input_rate,
           (unsigned long) output_rate, RESAMPLER_NUM_TAPS, resampleGetNumFilters(resampler_),
           resampleInterpolationUsed(resampler_) ? "ON" : "OFF");
  return true;
}

size_t audio_resample_process(const int16_t *in, size_t in_frames, int16_t *out, size_t out_capacity) {
  if (resampler_ == nullptr || !active_) {
    return 0;
  }

  size_t in_samples = in_frames * (size_t) current_channels_;
  size_t out_samples = out_capacity * (size_t) current_channels_;
  ...
  for (size_t i = 0; i < in_samples; i++) {
    float_in_[i] = (float) in[i] * (1.0f / 32768.0f);
  }

  ResampleResult result =
      resampleProcessInterleaved(resampler_, float_in_, (int) in_frames, float_out_, (int) out_capacity,
                                 fixed_ratio_);

  /* float -> int16 with clamp */
  size_t out_total = (size_t) result.output_generated * (size_t) current_channels_;
  for (size_t i = 0; i < out_total; i++) {
    float s = float_out_[i] * 32768.0f;
    if (s > 32767.0f) {
      s = 32767.0f;
    } else if (s < -32768.0f) {
      s = -32768.0f;
...
void audio_resample_reset(void) {
  if (resampler_ != nullptr) {
    resampleReset(resampler_);
  }
}

void audio_resample_destroy(void) {
  if (resampler_ != nullptr) {
    resampleFree(resampler_);
    resampler_ = nullptr;
  }
  airplay_free(float_in_);
  float_in_ = nullptr;
...
size_t audio_resample_max_output(size_t in_frames) {
  if (!active_ || current_input_rate_ == 0) {
    return in_frames;
  }
  return (size_t) ((double) in_frames * fixed_ratio_ + 2);
}
