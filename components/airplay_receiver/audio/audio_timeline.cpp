...
// engine (engine v2).
//
// Port of rbouteiller/airplay-esp32 main/audio/audio_timeline.c (PR #130) into
// the ESPHome component. The upstream logic is preserved exactly; the
// differences are:
//   * everything is wrapped in esphome::airplay_receiver;
//   * logging uses esphome/core/log.h (ESP_LOGW) with a file-scope TAG;
//   * ALL heap routes through airplay_alloc/airplay_calloc/airplay_free
//     (../allocator.h) — never malloc/calloc/new/free. `desc` (descriptors,
//     walked under a spinlock) stays internal DRAM (realtime=true); `pcm_pool`
//     (large decoded-frame payload pool) is PSRAM-first (realtime=false),
//     mirroring upstream's CONFIG_SPIRAM heap_caps_malloc(MALLOC_CAP_SPIRAM)
//     / malloc() split.
//
// The FreeRTOS binary semaphore (`space_available`) is a resource created and
// destroyed through xSemaphoreCreateBinary()/vSemaphoreDelete() — not heap
// routed through airplay_*, exactly as upstream.
...
static const char *const TAG = "audio_timeline";
...
esp_err_t audio_timeline_init(audio_timeline_t *t, uint16_t capacity,
  ...
  /* Large slow decoded-frame payload pool: PSRAM-first with internal fallback
   * (realtime=false). Replaces upstream's CONFIG_SPIRAM
   * heap_caps_malloc(MALLOC_CAP_SPIRAM) / malloc() fallback. */
  const size_t pcm_bytes = t->pool_pcm_samples * sizeof(int16_t);
  t->pcm_pool = (int16_t *)airplay_alloc(pcm_bytes, /*realtime=*/false);
