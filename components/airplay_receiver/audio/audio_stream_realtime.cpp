...
// (data + control), parses RTP headers, recovers the sequence-number stream and
// drives the AirTunes NACK retransmission protocol, then hands each accepted
// packet to the decrypt + decode stage (audio_stream_process_frame).
//
// SHARED CONTRACT:
//   * namespace esphome::airplay_receiver
//   * ALL heap via airplay_alloc/airplay_free (../allocator.h) — the 2 KiB
//     receive packet buffers go to PSRAM (realtime=false), matching upstream's
//     SPIRAM placement;
//   * logs via esphome/core/log.h + file-scope TAG;
...
// Task stack sizes: prefer the platform profile (platform/<variant>/config.h)
// when a native build put it on the include path, else the upstream values that
// are inlined there. The ESPHome external-component build does not copy the
// two-level platform headers, so the fallbacks below are what actually build.
...
#define AUDIO_RECV_STACK_SIZE AIRPLAY_TASK_STACK_AUDIO_RECV
#define AUDIO_CTRL_STACK_SIZE AIRPLAY_TASK_STACK_AUDIO_CTRL
...
// Audio tasks are pinned to the core the platform profile reserves for audio
// (core 1 on the dual-core targets). On a single-core build use core 0.
#if defined(CONFIG_FREERTOS_UNICORE)
#define AUDIO_TASK_CORE 0
#elif defined(AIRPLAY_CORE_AFFINITY_AUDIO)
#define AUDIO_TASK_CORE AIRPLAY_CORE_AFFINITY_AUDIO
#else
...
// Packet buffers are allocated once from PSRAM (via airplay_*). Tasks are
// created dynamically: restartable static TCBs can be corrupted if a new task
// reuses the TCB before the idle task has removed the old self-deleted task
// from FreeRTOS lists.
static uint8_t *s_recv_packet_buf;
static uint8_t *s_ctrl_packet_buf;

static const char *const TAG = "audio_rt";
...
static bool realtime_receive_packet(audio_stream_t *stream, uint8_t *packet,
  ...
  // Decrypt through the injected CryptoModule (audio_crypto.c is not ported).
  // The component only supports ChaCha20-Poly1305; NOT_SET means plaintext.
  if (stream->encrypt.type != AudioEncryptType::NOT_SET &&
      state->decrypt_buffer) {
    if (!state->crypto) {
      ESP_LOGE(TAG, "Decrypt requested but no CryptoModule injected");
      state->stats.decrypt_errors++;
      state->stats.packets_dropped++;
      return true;
    }
    int decrypted_len = state->crypto->audio_decrypt_rtp(
...
static void receiver_task(void *pvParameters) {
  ...
  if (!s_recv_packet_buf) {
    s_recv_packet_buf =
        (uint8_t *) airplay_alloc(MAX_RTP_PACKET_SIZE, false);  // PSRAM
  }
...
static void control_receiver_task(void *pvParameters) {
  ...
  if (!packet) {
    ...
    state->control_task_handle = nullptr;
    vTaskDelete(nullptr);
    return;
  }
  struct sockaddr_in src_addr;
...
static bool realtime_wait_for_tasks_stopped(audio_receiver_state_t *state,
  ...
  return state->task_handle == nullptr && state->control_task_handle == nullptr;
}
