...
// main/audio/audio_scheduler.c).
//
// Turns decoded / chain PCM frames (committed to the timeline by the
// decode+queue stage) into playout reservations and computes the render window
// the audio output backend pulls from: given the sender-clock position for the
// current callback, it finds a contiguous playable island on the timeline
// (starting a fresh preroll or falling back to a short degraded window), keeps
// cursor_rtp advancing with the output, and runs the drift servo that closes
// the loop between the output crystal and the sender's clock by trimming one
// sample at a time.  Emits silence (never errors) until it has a playable
// window.
//
// SHARED CONTRACT: namespace esphome::airplay_receiver; ALL heap via airplay_*
// (../allocator.h); logs via esphome/core/log.h + a per-file TAG; upstream
// function signatures preserved.
//
// This file performs NO heap allocation (the caller owns the scheduler struct)
// and emits NO diagnostics logs, so it neither includes ../allocator.h nor
// defines a TAG — consistent with the stream base, which likewise has no
// diagnostics of its own.  TYPE SOURCES: audio_timeline_t (audio_timeline.h)
// and audio_clock_map_t (audio_clock_map.h) are owned by other agents and are
// pulled in via audio_scheduler.h; nothing here redefines them.
